...
class Song
{
	public static function grabSong(songID:String = 'Epi Penned', jsonName:String = 'hard'):SongMap
	{
		final songPath:String = 'songs/$songID/$jsonName.json'; // assets/song/thing.json for example
		trace(songPath);
		...
		if (Paths.exists(songPath))
		{
			...
			json.notes.sort(function(event1, event2) return Math.floor(event1.time - event2.time));
			json.players[1] ??= 'gf';
			return json;
		}
		return {
